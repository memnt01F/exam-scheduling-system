...
router.post("/set-passwords", async(_req, res) => {
    try {
        ...
        const result = await User.updateMany({ password: { $exists: false } }, { $set: { password: hash } });
        res.json({
...
router.put("/:id", async(req, res) => {
    try {
        ...
        await AuditLog.create({
            action: "UPDATE_USER",
            user: req.body && req.body.updatedBy ? req.body.updatedBy : "admin",
            role: "admin",
...
router.delete("/:id", async(req, res) => {
    try {
        ...
        await AuditLog.create({
            action: "DELETE_USER",
            user: req.body && req.body.deletedBy ? req.body.deletedBy : "admin",
            role: "admin",
